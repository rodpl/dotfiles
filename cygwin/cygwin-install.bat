...
SET LOCALDIR=%ROOTDIR%/Downloads
...
setup-x86_64 --quiet-mode --no-desktop --download --local-install --no-verify -s %SITE% -l "%LOCALDIR%" -R "%ROOTDIR%" -B -W
ECHO.
ECHO.
ECHO *** INSTALLING CUSTOM PACKAGES
setup-x86_64 -q -d -D -L -X -s %SITE% -l "%LOCALDIR%" -R "%ROOTDIR%" -P %PACKAGES% -B -W
