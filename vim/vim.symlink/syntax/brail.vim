" Vim syntax file
" Language:	Brail
" Maintainer:	Daniel Dabrowski (rod@sgh.waw.pl)
" Updated:	2007-10-08
"
...
syn keyword AspVBSStatement contained component
...
syn region  BrailInsideHtmlTags keepend matchgroup=Delimiter start=+${+ end=+}+ contains=@AspVBScriptTop, AspVBSFold
syn region  BrailInsideHtmlTags keepend matchgroup=Delimiter start=+<%=\=+ end=+%>+ contains=@AspVBScriptTop, AspVBSFold
syn region  BrailInsideHtmlTags keepend matchgroup=Delimiter start=+<script\s\+language="\=vbscript"\=[^>]*\s\+runatserver[^>]*>+ end=+</script>+ contains=@AspVBScriptTop


" Synchronization
" syn sync match AspVBSSyncGroup grouphere BrailInsideHtmlTags "<%"
